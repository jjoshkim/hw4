...
import { Keyboard, TextInput, Button, StyleSheet, Text, View } from 'react-native';
...
export default class App extends React.Component {
  ...
  async getWeather() {
    ...
    // current conditions
    let current = response.weather.currently;
    let currentIcon = icon(current.icon);
    let currentTemp = Math.round(current.apparentTemperature);
    let currentSummary = current.summary;

    // forecast

    let forecast = [];
    let nextDay = response.weather.daily.data;
    for(let i=0; i<5; i++) {
      let nextDayIcon = icon(nextDay[i].icon);
      let nextDayTemp = Math.round(nextDay[i].temperatureHigh);
      let nextDayInfo = {icon: nextDayIcon, temperature: nextDayTemp};
      forecast.push(nextDayInfo);
    }
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.currentWeather}>
          <Text style={styles.currentIcon}>{this.state.currentIcon && (<Icon size={100} color="#000" name={this.state.currentIcon} />)}</Text>
